...
class BillSpec extends FunSuite with MockitoSugar{
  ...
  test("display bill"){
    ...
    when(mockGeneralItem.getBill(Map("apple"->2,"surfexcel"->1,"chocolate"->5),Map("apple"->200,"surfexcel"->58,"chocolate"->25))).thenReturn(597.574999999999)
    val actualResult=testBill.displayBill(mockMedicalItem,mockGeneralItem)
    val expectedResult=687.574999999999
